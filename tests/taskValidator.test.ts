...
describe('TaskValidator', () => {
  ...
  describe('isValidTaskElement', () => {
    ...
    it('should validate a task element without a span (v2 has none)', () => {
  ...
  describe('isTaskCardTaskMarkdown (v2 detection)', () => {
    it.each([
      '- [ ] plain tagged task #TaskCard',
      '- [x] done tagged task #TaskCard ^tc-abc123',
      '- [ ] fielded task #TaskCard [due:: 2026-07-10] ^tc-abc123',
      '- [ ] emoji task #TaskCard 📅 2026-07-10',
      '  - [ ] indented tagged task #TaskCard'
    ])('accepts %s', (line) => {
      expect(taskValidator.isTaskCardTaskMarkdown(line)).toBe(true);
    });

    it.each([
      '- [ ] untagged task',
      '- [ ] longer tag #TaskCardX',
      'not a task at all #TaskCard',
      '- plain list item #TaskCard'
    ])('rejects %s', (line) => {
      expect(taskValidator.isTaskCardTaskMarkdown(line)).toBe(false);
    });

    it('matches a custom indicator tag containing a regex metacharacter, without double-escaping', () => {
      mockSettingStore.set({
        parsingSettings: {
          indicatorTag: 'TaskCard.v2',
          markdownStartingNotation: '%%*',
          markdownEndingNotation: '*%%',
          markdownSuffix: ' .'
        }
      ...
      expect(
        taskValidator.isTaskCardTaskMarkdown('- [ ] x #TaskCard.v2')
      ).toBe(true);
      expect(
        taskValidator.isTaskCardTaskMarkdown('- [ ] x #TaskCardXv2')
      ).toBe(false);
    });
  });

  describe('hasBlockId / hasLegacyNotation', () => {
    it('detects a trailing block id', () => {
      expect(
        taskValidator.hasBlockId('- [ ] task #TaskCard ^tc-abc123')
      ).toBe(true);
      expect(taskValidator.hasBlockId('- [ ] task #TaskCard')).toBe(false);
    });

    it('detects legacy notations', () => {
      expect(
        taskValidator.hasLegacyNotation(
          '- [ ] t #TaskCard<span style="display:none">{"id":"x"}</span> .'
        )
      ).toBe(true);
      expect(
        taskValidator.hasLegacyNotation('- [ ] t #TaskCard %%*priority:1*%%')
      ).toBe(true);
      expect(
        taskValidator.hasLegacyNotation('- [ ] t #TaskCard [due:: 2026-07-10]')
      ).toBe(false);
    });
  });

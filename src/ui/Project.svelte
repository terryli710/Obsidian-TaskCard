...
<script lang="ts">
  ...
  // auto-subscription: disposed by Svelte when the component is destroyed
  ...
  // The popup is a picker, not a text editor: dismissing it (outside click or
  // Escape) selects nothing and closes. Attached as an action so it covers
  // every path that mounts the popup. Capture phase, so a click swallowed by
  // another widget's stopPropagation still dismisses it.
  function dismissOnOutsideInteraction(node: HTMLElement) {
    const onDocumentMousedown = (event: MouseEvent) => {
      const target = event.target;
      if (
        target instanceof Node &&
        (node.contains(target) || projectWrapper?.contains(target))
      ) {
        return;
      }
      closeProjectPopup();
  ...
  function searchProject(evt: MouseEvent | KeyboardEvent, projectName: string) {
    ...
    // Obsidian exact phrases escape backslashes and quotes with `\`. Keep the
    // visible native-search query aligned with the v2 on-disk field instead
    // of the retired hidden JSON (`"name":"..."`).
    const escapedProjectName = projectName
      .replace(/\\/g, '\\\\')
      .replace(/"/g, '\\"');
    // @ts-ignore
    taskSyncManager.plugin.app.internalPlugins
      .getPluginById('global-search')
      .instance.openGlobalSearch(`task:"project:: ${escapedProjectName}"`);
  }
</script>
...
{#if params.mode === "single-line"}
  <div class="task-card-project">
    {#if project}
      ...
      ></span>
      <span class="project-name-quiet single-line">{project.name}</span>
    {/if}
  </div>
{:else}
  <div class="project-wrapper" bind:this={projectWrapper}>
    {#if taskSyncManager.getTaskCardStatus('projectStatus') === 'selecting' && availableProjects.length > 0}
      <div class="project-popup" bind:this={projectPopup} use:dismissOnOutsideInteraction>
        {#if project}
            <div
    ...
    {:else}
      {#if project}
        <div class="task-card-project">
          <div class="project-name project-name-quiet"
...
<style>
  ...
  /* corner project (pillar-3 refresh): plain muted text next to the color dot,
     no tag chrome — the project reads as context, not as another chip */
  .project-name.project-name-quiet {
    background-color: transparent;
    border: none;
  ...
  .project-name.project-name-quiet:hover {
    background-color: transparent;
    border: none;
  ...
  .project-name-quiet.single-line {
    color: var(--text-muted);
    font-size: var(--font-ui-smaller);
    max-width: 8em;
    overflow: hidden;

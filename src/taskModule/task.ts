...
export interface TaskProperties {
  ...
  /** YYYY-MM-DD stamp written as `[completion:: ...]` when completed via TaskCard */
  completionDate?: string | null;
  metadata?: {
    taskDisplayParams?: TaskDisplayParams | null;
    syncMappings?: SyncMappings;
    /** raw `[key:: value]` strings TaskCard doesn't understand, preserved on rewrite */
...
export class ObsidianTask implements TaskProperties {
  ...
  constructor(props?: Partial<ObsidianTask>) {
    ...
    this.recurrence = props?.recurrence || null;
  ...
  hasSchedule(): boolean {
    ...
    // string is optional; fall back to the date when it's absent
    return !!(this.schedule.string?.trim() || this.schedule.date);
  }

  hasDue(): boolean {
    if (!this.due) return false;
    // string is optional; fall back to the date when it's absent
    return !!(this.due.string?.trim() || this.due.date);
  }
  ...
  hasRecurrence(): boolean {
    return !!(this.recurrence && this.recurrence.trim());
  }

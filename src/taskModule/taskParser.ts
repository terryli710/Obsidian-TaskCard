...
import {
  ...
  extractEmojiFields,
  extractInlineFields,
  parseIsoScheduleValue,
  priorityWordToNumber
} from './fieldSyntax';
import { parseScheduleInput } from './temporal';

const V1_HIDDEN_SPAN = /<span( class="[^"]+")? style="display:none">\{.*\}<\/span>/;
...
export class TaskParser {
  ...
  parseTaskEl(taskEl: HTMLElement): ObsidianTask {
    ...
    task.priority = attributes.priority || 4;
    ...
    task.recurrence = attributes.recurrence || null;
  ...
  parseTaskMarkdown(taskMarkdown: string, noticeFunc: (msg: string) => void = null): ObsidianTask {
    ...
    // v2 identity: a trailing block id is the task's durable id
    const { id: blockId, rest: lineWithoutId } = extractBlockId(taskMarkdown);
    if (blockId) task.id = blockId;
    taskMarkdown = lineWithoutId;

    // v2 attributes: Dataview inline fields ([key:: value] / (key:: value)),
    // with unknown keys preserved verbatim for round-tripping
    const inlineFields = extractInlineFields(taskMarkdown);
    taskMarkdown = inlineFields.rest;

    // Tasks-plugin emoji dialect (📅 ⏳ 🔁 🔺 …); explicit fields win over it.
    // 🔁 only consumes its trailing text when that text is a valid rule.
    const emojiFields = extractEmojiFields(
      taskMarkdown,
      (rule) => parseRecurrenceRule(rule) !== null
    );
    taskMarkdown = emojiFields.rest;

    const foreignFields = [...inlineFields.foreign, ...emojiFields.foreign];
    if (foreignFields.length > 0) {
      task.metadata.foreignFields = foreignFields;
    }
    ...
    if (emojiFields.priority !== null) {
      task.priority = emojiFields.priority;
    }
    ...
    for (const match of matches) {
      ...
      switch (attributeName) {
        ...
        case 'duration':
          ...
          break;
        case 'recurrence':
          task.recurrence = tryParseAttribute('recurrence', this.parseRecurrence.bind(this), attributeValue, 'string');
          break;
        case 'project':
    ...
    // Emoji-derived values first, canonical fields last so they win
    this.applyKnownFields(task, emojiFields.known, errors);
    this.applyKnownFields(task, inlineFields.known, errors);
  ...
  /** Apply canonical `key → raw value` pairs (from inline fields or emoji). */
  private applyKnownFields(
    task: ObsidianTask,
    ...
    for (const [key, rawValue] of Object.entries(known)) {
      if (rawValue === undefined) continue;
      switch (key as KnownFieldKey) {
        case 'priority':
          applyOrReport(key, rawValue, priorityWordToNumber(rawValue), (v) => (task.priority = v));
          break;
        case 'due':
          applyOrReport(
            key,
            rawValue,
            parseIsoScheduleValue(rawValue) ?? this.parseSchedule(rawValue),
            (v) => (task.due = v)
          );
          break;
        case 'scheduled':
          ...
          break;
        case 'duration':
          applyOrReport(key, rawValue, this.parseDuration(rawValue), (v) => (task.duration = v));
          break;
        case 'repeat':
  ...
  /**
   * Parse a task in any dialect TaskCard reads: v2 fields / Tasks emoji /
   * legacy `%%*` notation (all handled by parseTaskMarkdown), or the legacy
   * v1 hidden-span format.
   */
  parseAnyTaskMarkdown(
    taskMarkdown: string,
    noticeFunc: (msg: string) => void = null
  ): ObsidianTask {
    const firstLine = taskMarkdown.split('\n', 1)[0];
    if (V1_HIDDEN_SPAN.test(firstLine)) {
      return this.parseFormattedTaskMarkdown(taskMarkdown);
    }
    return this.parseTaskMarkdown(taskMarkdown, noticeFunc);
  }

  parseAnyTaskFromFileLines(lines: string[]): ObsidianTask {
    ...
    return this.parseAnyTaskMarkdown(taskMarkdown);
  }
  ...
  parseFormattedTaskMarkdown(taskMarkdown: string): ObsidianTask {
    ...
    task.recurrence = parseJSONAttribute(metadata['recurrence'], 'recurrence', null);
  ...
  parseSchedule(scheduleString: string): ScheduleDate | null {
    return parseScheduleInput(scheduleString);
  }

  parseDuration(durationString: string): Duration | null {
    const durationInMinutes = parse(durationString, 'm');
    if (durationInMinutes === null || durationInMinutes === undefined || isNaN(durationInMinutes)) {
      return null;
    }
  ...
  parseRecurrence(recurrenceString: string): string | null {
